...
const core$1 = require('@graphql-toolkit/core');
...
const loadSchema = async (schemaPointers, config) => {
    try {
        ...
        const schema = await core$1.loadSchema(schemaPointers, {
            assumeValidSDL: true,
            loaders,
            sort: true,
...
const loadDocuments = async (documentPointers, config) => {
    ...
    const loadedFromToolkit = await core$1.loadDocuments(documentPointers, {
...
function parseArgv(argv = process.argv) {
    return setCommandOptions(new commander.Command().usage('graphql-codegen [options]')).parse(argv);
}
                        ...
                        return new Listr([
                            {
                                ...
                                task: wrapTask(async () => {
                                    ...
                                    outputSchema = graphql.parse(common.printSchemaWithDirectives(outputSchemaAst));
                                }, filename),
                            },
                            {
...
const createWatcher = (initalContext, onNext) => {
    ...
    schemas.forEach((schema) => {
        if (isGlob(schema) || common.isValidPath(schema)) {
            files.push(schema);
        }
    });
