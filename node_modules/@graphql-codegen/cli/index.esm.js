...
import { resolve, join, delimiter, sep, dirname, isAbsolute, relative } from 'path';
import { printSchemaWithDirectives, isValidPath } from '@graphql-toolkit/common';
...
import { loadSchema as loadSchema$1, loadDocuments as loadDocuments$1 } from '@graphql-toolkit/core';
...
import { readFileSync, writeFileSync, statSync, unlink } from 'fs';
...
function setCommandOptions(commandInstance) {
    return commandInstance
        .allowUnknownOption(true)
        .option('-c, --config <path>', 'Path to GraphQL codegen YAML config file, defaults to "codegen.yml" on the current directory')
        .option('-w, --watch [value]', 'Watch for changes and execute generation automatically. You can also specify a glob expreession for custom watch list.')
...
function parseArgv(argv = process.argv) {
    return setCommandOptions(new Command().usage('graphql-codegen [options]')).parse(argv);
}
...
const defaultLoader = (mod) => import(mod);
...
export { CodegenContext, CodegenExtension, cliError, createContext, ensureContext, executeCodegen, findAndLoadGraphQLConfig, generate, init, loadContext, parseArgv, runCli, setCommandOptions, updateContextWithCliFlags };
