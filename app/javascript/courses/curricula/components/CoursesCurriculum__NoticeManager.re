...
let courseCompletedMessage = course => {
  ...
  let title =
    ...
    | None => "Congratulations! You have completed all modules in this course."
    };
...
let levelUpBlockedMessage = currentLevelNumber => {
  ...
  let description =
    "You have submitted all modules in level "
    ++ string_of_int(currentLevelNumber)
    ++ ", but one or more submissions are pending review by a coach. You need to get a passing grade on all modules to level up.";
...
let levelUpLimitedMessage = (currentLevelNumber, minimumRequiredLevelNumber) => {
  ...
  let description =
    ...
    ++ " that are failed, or are pending review by a coach. You'll need to pass all modules in Level "
    ++ minimumRequiredLevel
...
let renderLevelUp = course => {
  ...
  let description = "Congratulations! You have successfully completed all modules required to level up. Click the button below to proceed to the next level. New challenges await!";
  <div
