...
let reducer = (state, action) =>
  switch (action) {
  | RefreshData(teams) => {...state, teams, selectedTeams: []}
  | SelectTeam(team) => {
      ...state,
  ...
  | DeselectTeam(team) => {
      ...state,
      selectedTeams:
        state.selectedTeams |> List.filter(s => Team.id(s) !== Team.id(team)),
    }
...
let make =
  ...
  let (state, send) =
    React.useReducer(
      reducer,
      {teams, students, selectedTeams: [], searchString: ""},
    );
    ...
    <div className="max-w-3xl mx-auto w-full">
      {state.teams |> List.length > 0
         ...
         : <div className="shadow bg-white rounded-lg mb-4 p-4">
             {"No inactive user matches your search criteria." |> str}
           </div>}
